...
const MessageInput = () => {
  ...
  const { isGroupChat, selectedConversation, replyTo, clearReplyTo } = useConversation();
  ...
  const handleSubmit = async () => {
    ...
    if (!selectedConversation) { toast.error("No conversation selected"); return; }
    if (!socket)               { toast.error("Not connected to server");   return; }
    if (uploading)             { toast.error("Please wait, file is uploading…"); return; }
    ...
    try {
      if (isGroupChat) {
        await sendGroupMessage(message.trim(), selectedConversation._id, options);
      } else {
        await sendMessage(message.trim(), selectedConversation._id, options);
      }
      setMessage("");
      setAttachment(null);
      clearReplyTo();
  ...
  return (
    <div className="border-t border-gray-200/80 dark:border-gray-700/50
      ...
      {(replyTo || attachment || uploading) && (
        <div className="mb-2.5 flex flex-col gap-1.5 rounded-xl border border-indigo-200/80 dark:border-indigo-800/50
          bg-indigo-50/80 dark:bg-indigo-900/20 px-3 py-2 text-xs">
          {replyTo && (
            <PreviewRow
              label="Reply"
              value={replyTo.message || replyTo.attachment?.fileName || "Attachment"}
      ...
      <div className="flex items-center gap-2 rounded-2xl border-2 border-gray-200 dark:border-gray-700
        ...
        <input
          ...
          placeholder={isGroupChat ? "Message this group…" : "Type a message…"}
...
const PreviewRow = ({ label, value, onClear }) => (
  <div className="flex items-center justify-between gap-3">
    <div className="flex min-w-0 items-center gap-2 text-indigo-700 dark:text-indigo-300">
      ...
      <span className="truncate text-gray-600 dark:text-gray-400">{value}</span>
    </div>
    <button
      type="button"
      onClick={onClear}
