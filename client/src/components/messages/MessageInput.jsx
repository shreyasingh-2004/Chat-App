...
const MessageInput = () => {
  ...
  // Local preview shown while a file is selected/uploading
  ...
  const uploadFile = (file) => {
    ...
    // Create instant local preview for images/videos
    const previewUrl =
    ...
    (async () => {
      try {
        ...
        const data = await new Promise((resolve, reject) => {
          ...
          xhr.open(
            "POST",
            `${baseUrl}/api/messages/upload-media`
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Tell server that typing has stopped
      if (socket && selectedConversation) {
        socket.emit(
          "stopTyping",
          isGroupChat
  ...
  const canSend =
    (message.trim() || attachment?.url) &&
    !loading;

  const PendingIcon = pending
    ? fileIconFor(pending.mediaType)
    : File;
